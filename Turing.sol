...
contract Turing is TuringInterface, InternalModule {
    ...
    function SubDepositedLimitCurrent(uint256 v) external APIMethod {
    ...
    function PowerOn() external APIMethod {
    ...
    function PowerOn() external OwnerOnly {

        ownerDisable = false;
        enable = true;
    }

    function PowerOff() external OwnerOnly {

        ownerDisable = true;
        enable = false;
    }

    function IPTS(uint256 i, uint256 p, uint256 t, uint256 s) external OwnerOnly {
        I = i;
        P = p;
        T = t;
        S = s;
